  ...
  def render
    div class: "mt-6 grid grid-cols-12 gap-6" do
      if (user = operation.record)
        ...
        div class: "col-span-12" do
          mount Shared::Field, operation.readonly_team, "#{pluralize(user.teams.size, "team membership")}", &.text_input(attrs: [:disabled], value: user.teams.map(&.name).join(", "), append_class: "bg-gray-50")
        end
      end
